...
output "optional_dns_name" {
  value = "${element(
    concat(azurerm_dns_a_record.optional_ops_manager_dns.*.name, [""]),
    0,
    )}.${element(
    concat(
      azurerm_dns_a_record.optional_ops_manager_dns.*.zone_name,
      [""],
    ),
    0,
  )}"
...
output "optional_ops_manager_public_ip" {
  value = element(
    concat(
      azurerm_public_ip.optional_ops_manager_public_ip.*.ip_address,
      [""],
    ),
    0,
  )
