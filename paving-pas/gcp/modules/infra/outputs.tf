...
output "blobstore_gcp_service_account_key" {
  value = base64decode(
    element(
      concat(google_service_account_key.blobstore.*.private_key, [""]),
      0,
    ),
  )
  sensitive = true
...
output "blobstore_service_account_email" {
  value = element(concat(google_service_account.blobstore.*.email, [""]), 0)
}

output "blobstore_service_account_project" {
  value = element(concat(google_service_account.blobstore.*.project, [""]), 0)
}
