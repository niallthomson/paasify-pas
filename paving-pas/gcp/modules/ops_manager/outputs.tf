...
output "ops_manager_private_ip" {
  value = element(
    concat(
      google_compute_instance.ops-manager.*.network_interface.0.address,
      [""],
    ),
    0,
  )
...
output "service_account_key" {
  sensitive = true
  value = base64decode(
    google_service_account_key.opsman_service_account_key.private_key,
  )
}

output "sql_db_name" {
  value = element(concat(google_sql_database.opsman.*.name, [""]), 0)
}

output "sql_username" {
  value = element(concat(random_id.opsman_db_username.*.b64, [""]), 0)
}

output "sql_password" {
  sensitive = true
  value     = element(concat(random_id.opsman_db_password.*.b64, [""]), 0)
}
