resource "google_compute_ssl_certificate" "certificate" {
  ...
  description = "SSL certificate pair"
  certificate = module.pave.cert_full_chain
  private_key = module.pave.cert_key
}
...
resource "google_compute_instance_group" "http-lb" {
  name = "${var.env_name}-http-lb-${count.index}"
...
resource "google_compute_http_health_check" "http-lb" {
  name                = "${var.env_name}-http-lb-health-check"
  port                = 8080
  request_path        = "/health"
...
resource "google_compute_forwarding_rule" "tcp-lb" {
  name        = "${var.env_name}-tcp-lb-${count.index}"
...
resource "google_compute_address" "websocket-lb" {
  name = "${var.env_name}-websocket-lb"
}

resource "google_compute_forwarding_rule" "websocket-lb-80" {
  name        = "${var.env_name}-websocket-lb-80"
...
resource "google_compute_forwarding_rule" "websocket-lb-443" {
  name        = "${var.env_name}-websocket-lb-443"
  ip_address  = google_compute_address.websocket-lb.address
  target      = google_compute_target_pool.websocket-lb.self_link
  port_range  = "443"
  ip_protocol = "TCP"
